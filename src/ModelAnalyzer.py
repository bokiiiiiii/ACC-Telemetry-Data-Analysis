...
class ModelAnalyzer:
    ...
    def analyze_action_distribution(self):
        ...
        print(f"  % Full Throttle (>0.9): {100 * np.sum(throttle > 0.9) / len(throttle):.1f}%")
    ...
    def analyze_reward_distribution(self):
        ...
        if len(positive_rewards) > 0:
            print(f"Positive Rewards: {len(positive_rewards)} ({100*len(positive_rewards)/len(rewards):.1f}%)")
            print(f"  Mean Positive: {np.mean(positive_rewards):.4f}")
        
        if len(negative_rewards) > 0:
            print(f"Negative Rewards: {len(negative_rewards)} ({100*len(negative_rewards)/len(rewards):.1f}%)")
    ...
    def plot_action_distributions(self, output_file=None):
        ...
        axes[0].axvline(np.mean(all_actions[:, 0]), color='red', linestyle='--', label='Mean')
        axes[0].legend()
        ...
        axes[1].axvline(np.mean(all_actions[:, 1]), color='red', linestyle='--', label='Mean')
        axes[1].legend()
        ...
        axes[2].axvline(np.mean(all_actions[:, 2]), color='red', linestyle='--', label='Mean')
        axes[2].legend()
    ...
    def plot_reward_distribution(self, output_file=None):
        ...
        ax1.axvline(np.mean(rewards), color='red', linestyle='--', label=f'Mean: {np.mean(rewards):.4f}')
        ax1.legend()
